...
INSERT INTO post (member_id, title, content, created_at, view_count) VALUES
    ...
    (4, '자바 static 키워드 정리', '클래스 멤버와 인스턴스 멤버의 차이를 정리했습니다.', '2026-06-16 08:15:00', 6),
...
SELECT p.id, p.member_id,
    ...
    p.content, p.created_at, p.view_count, m.name, COUNT(r.id) reply_count
	FROM post p
...
SELECT p.title, p.content, m.name 글작성자, m.email, 
	(SELECT name FROM member WHERE member.id = r.member_id) 댓글작성자, r.content
	FROM post p
	LEFT JOIN member m ON p.member_id = m.id
	LEFT JOIN reply r ON p.id = r.post_id;
...
INSERT INTO post (member_id, title, content, created_at, view_count) VALUES
    (3, '테스트 게시물입니다', '곧 삭제될 게시물입니다', '2026-06-16 17:57:00', 20);
...
UPDATE post
	SET title = '이 글은 현재 볼 수 없는 게시글입니다.',
        content = '이 글은 정책 위반으로 내용이 삭제되었습니다.'
	WHERE id = 2;




-- 5. 게시글 삭제 (문제 3번에서 등록한 게시글 삭제)

-- SELECT * FROM post ORDER BY id DESC LIMIT 1;
DELETE FROM post ORDER BY id DESC LIMIT 1;
