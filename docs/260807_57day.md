...
## 2교시: 클라우드 컴퓨팅과 Amazon Web Services 기초

# 클라우드 컴퓨팅과 Amazon Web Services 기초
...
- 자원 할당에 가상화 소프트웨어 조종으로 자동화되어 수 초 또는 수 분 내에 필요한 IT 자원을 생성하고 해제 가능

### 기존 인프라 운영 방식과의 특성 차이

**온프레미스**

- 기업이 자체 전산실이나 서버실 시설을 구축하고 물리 서버, 전력, 냉각 설비, 네트워크 장비를 직접 소요하여 유지 관리
- 하드웨어 구매를 위한 초기 고정 투자 비용이 소요되며, 자원 확장 시 견적 수립, 발주, 납품, 서버 랙 마운트 설치까지 수 주에서 수 개월의 지연 발생

**코로케이션**

- 기업 소유의 물리 서버 장비를 외부 전문 IDC(Internet Data Center) 공간에 입주시켜 전력, 항온항습 냉각 시설 및 물리 보안 관리만 위탁 이용
- 전산실 물리 시설 구축 비용은 절감, 하드웨어 소유권과 장비 유지보수 책임을 기업이 직접 담당하며 장기 랙 공간 임대 계약 방식 적용

**서버 호스팅**

- IDC 전문 업체의 물리 서버 장비를 월 단위 정액제 계약 형태로 임대 받아 이용
- 클라우드와 달리 물리 장비 세팅을 위한 작업 시간이 필요, 자원 자율 축소나 사용한 시간 단위 종량제 요금 정산이 지원되지 않음

### 클라우드의 주요 이점

![cloud](imgs/cloud.png)

**비즈니스 민첩성 향상 및 출시 시간 단축**

- 인프라 확보 절차가 소프트웨어 호출 방식으로 단축, 신속한 프로비저닝(Provisioning)을 바탕으로 신규 기능 테스트 및 서비스 출시 가속화
...
- 사전 대규모 고정 투자 없이 실제 가동한 리소스 사용 시간 및 트래픽 양에 대해서만 정산하는 가변 운영 비용 모델
...
- 하드웨어 고장 교체, 데이터 센터 물리 관리 부담을 경감하고 관리형 서비스(Managed Service) 활용을 통한 운영 복잡성 경감

## 주요 클라우드 제공업체 및 시장 점유율

### 글로벌 하이퍼스케일 벤더

- Amazon Web Services (AWS):
    - 2006년 상용 클라우드 서비스를 최초로 개척한 글로벌 리더
    - 가장 광범위한 인프라 거점과 최다 관리형 서비스 보유
- Microsoft Azure
    - 기존 기업형 윈도우 인프라, Active Directory 및 MS 제품군과의 연동성을 바탕으로 엔터프라이즈 B2B 시장 확장
- Google Cloud Platform (GCP):
    - 대규모 빅데이터 처리, 쿠버네티스 컨테이너 기술 및 인공지능 머신러닝 연구 환경에 특화된 서비스 환경 제공

### 글로벌 인프라 시장 점유율 및 동향 (2026 기준)
...
- 글로벌 시장 점유율 현황 (Synergy Research Group 2026년 최신 리서치 기준):
    ...
    - 기타: Alibaba Cloud, Oracle Cloud, IBM Cloud 및 인공지능(AI) 연산 자원에 특화된 신흥 전문 클라우드 3 (Neocloud) 기업들의 가파른 성장세 형성.

## Amazon Web Service (AWS)

- 아마존사가 전 세계 비즈니스 및 개발자에게 인터넷을 통해 IT 인프라와 솔루션을 종량제 기반으로 제공하는 세계 최대의 종합 클라우드 컴퓨팅 플랫폼
- 글로벌 데이터 센터 망을 기반으로 서버, 스토리지, 데이터베이스 등 200개 이상의 독힙 관리형 클라우드 서비스 제공

### 클라우드 서비스 제공 모델

- Iaas (Infrastructure as a Service)
    - 서버, 스토리지, 네트워킹 등 가상화된 기본 IT 인프라 리소스를 제공
    - 운영체제 설치, 미들웨어 구성 및 애플리케이션 관리는 사용자가 직접 담당하는 서비스 모델
    - Amazon EC2, Amazon VPC, Amazon EBS 등
- PaaS (Platform as a Service)
    - 개발 런타임 및 실행 환경을 플랫폼으로 제공
    - 하드웨어 및 OS 관리 부담 없이 애플리케이션 코드 작성과 서비스 배포에 집중 가능한 모델
    - AWS Elastic Beanstalk, AWS App Runner 등
- SaaS (Software as a Service)
    - 클라우드 제공업체가 관리하는 완성된 소프트웨어 애플리케이션을 웹 브라우저나 API를 통해 별도 설치 없이 즉시 이용하는 서비스 모델
    - Amazon WorkDocs, Amazon Chime, Amazon QuickSight

### 핵심 서비스 영역

- 컴퓨팅: 가상 서버 EC2 및 이벤트 기반 서버리스 컴퓨팅 Lambda 지원
- 스토리지: 무제한 객체 스토리지 S3 및 블록 레벨 저장소 EBS 제공
- 데이터베이스: 관리형 RDBMS RDS 및 Key-Value NoSQL DynamoDB 지원
- 네트워킹 및 콘텐츠 전송: 격리 가상 네트워크 VPC, 콘텐츠 전송망 CloudFront 및 DNS 서비스 Route S3 제공
- 보안 자격 증명: 자격 증명 접근 관리 IAM 및 통합 보안 정책 제공

![aws](imgs/aws.png)

## 3교시: AWS 글로벌 인프라스트럭처 아키텍처

# AWS 글로벌 인프라스트럭처 아키텍처
...
- 주요 서비스 이용자와의 지리적 거리를 최소화하여 네트워크 지연시간(Latency) 경감
...
- 지역별 수수료 정책 및 인프라 운용 차이 검토

## AWS Local Zones 및 엣지 네트워크

### AWS Local Zones (로컬 존)

- 컴퓨팅, 스토리지, 데이터베이스 등 핵심 AWS 서비스를 대도시 인구 밀집 지역 근처에 배치하여 초저지연 연산 환경을 제공하는 인프라 확장 형태
- 주요 리전의 부모 가용 영역과 고속 전용선으로 연결되어 온프레미스 워크로드(Workload)의 클라우드 연동 지원

### 엣지 로케이션 (Edge Location) 및 POP

- 사용자 근접 거점에 배치되어 최단 지연 시간으로 데이터 및 콘텐츠를 캐싱 전송하는 접속 지점 (Point of Presense)
- Amazon CloudFront CDN 및 Amazon Route 53 전역 DNS 서비스의 캐시 서버로 동작

![edge-location](imgs/edge-location.png)

### 2단계 데이터 캐싱 계층 메커니즘

- 엣지 로케이션 캐시: 전 세계 주요 도시에 위치하여 최종 사용자 요청에 실시간 캐시 데이터 즉시 응답
- 리전 엣지 캐시 (Regional Edge Cache): 엣지 로케이션과 오리진(Origin) 서버(S3, EC2 등) 위치하는 중간 캐싱 계층
- 자주 액세스되지 않아 엣지 로케이션에서 만료된 콘텐츠를 리전 엣지 캐시 수준에서 재확인하여 오리진 서버의 직접 트래픽 부하 경

## 4교시: AWS 계정 생성 및 EC2 인스턴스 생성 실습

# AWS 계정 생성 및 EC2 인스턴스 생성 실습

- WS 계정 생성시 해외 결제가 가능한 신용카드나 체크카드 필요
...
## **개발 프로젝트 빌드 및 EC2 파일 전송**

### **키 파일 복사 및 Git 보안 관리 (.gitignore 설정)**
...
### **Spring Boot 애플리케이션 빌드 및 배포 파일 전송**
...
## **애플리케이션 포그라운드/백그라운드 실행 및 접속 테스트**
...
### **80번 HTTP 포트를 8080번 애플리케이션 포트로 리다이렉트**
